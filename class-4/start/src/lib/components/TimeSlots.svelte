<script>
	...
	/**
	 * Because the API does not support schedules we have to combine multiple request to handle our own.
	...
	const timeSlotsApiRoute = `http://localhost:3012/api/v1/timeslots/`;
	const allAppointmentsOnOneDay = `http://localhost:3012/api/v1/appointments?day=248`;
	...
	/**
	 * This function checks if the timeslot is in an appointment
	 * @param timeSlotId
	 * @param data
	 */
	...
	onMount(async () => {
		...
		// we have to execute multiple requests, so promisify the requests
